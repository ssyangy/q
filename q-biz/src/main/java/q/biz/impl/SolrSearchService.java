...
public class SolrSearchService implements SearchService {
		...
		runThread(new Runnable() {
			public void run() {
				try {
					...
					log.error("Search Engine is out of use and the weibo " + data.getId());
				}
			}
		});
	}
	...
	@Override
	public void updatePeople(final People data) throws Exception {
		runThread(new Runnable() {
			public void run() {
				try {
	...
	@Override
	public void updateGroup(final Group data) {
		runThread(new Runnable() {
			public void run() {
				try {
					...
					log.error("Search Engine is out of use and the group " + data.getId());
				}
			}
		});
	...
	private void runThread(Runnable a) {
		new Thread(a).start();
	}
