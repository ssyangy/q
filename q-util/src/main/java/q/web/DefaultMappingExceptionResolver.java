 ...
 * @author seanlinwang
 * @date May 28, 2011 copy from org.springframework.web.servlet.handler.SimpleMappingExceptionResolver
 * @see org.springframework.web.servlet.DispatcherServlet
 */
public class DefaultMappingExceptionResolver extends AbstractHandlerExceptionResolver {
	...
	/**
	 * Set the mappings between exception class names and error view names. The exception class name can be a substring, with no wildcard support at present. A value of "ServletException" would match <code>javax.servlet.ServletException</code> and subclasses, for example.
	 * <p>
	 ...
	 *            HTTP status code value, for example 500 ({@link HttpServletResponse#SC_INTERNAL_SERVER_ERROR}) or 404 ({@link HttpServletResponse#SC_NOT_FOUND})
	 ...
	 * <p>
	 * This can be either set to a different attribute name or to <code>null</code> for not exposing an exception attribute at all.
	 * 
	 ...
	 * <p>
	 * May be overridden in subclasses, in order to apply specific exception checks. Note that this template method will be invoked <i>after</i> checking whether this resolved applies ("mappedHandlers" etc), so an implementation may simply proceed with its actual exception handling.
	 * 
	 * @param request
	 ...
	 * @param handler
	 *            the executed handler, or <code>null</code> if none chosen at the time of the exception (for example, if multipart resolution failed)
	 * @param ex
	 ...
	 * @return the HTTP status code to use, or <code>null</code> for the servlet container's default (200 in case of a standard error view)
